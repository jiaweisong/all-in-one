...
通过服务发现，配合内置的命名空间（Namespace）规则，**API**可以把请求url解析映射到该属于该命名空间所属的服务接口。

在Micro体系中，服务都会有自己的命名空间，而**API**的默认命名空间是**go.micro.api**。通常，在其后的提供具体API接口的服务，我们会默认按照
...
Micro内部有将http请求路径映射到服务的机制，映射规则可以从下表中介绍
